...
export const importConfig = {
    ...
    dataSourceUrl: process.env.IMPORT_DATA_SOURCE_URL || "https://globalracecalendar.com/imotko/delta.json",

    // System user and agency IDs
    systemUserId: process.env.IMPORT_SYSTEM_USER_ID,
    defaultAgencyId: process.env.IMPORT_DEFAULT_AGENCY_ID || "cma2k2yrv0002l2043m8w706g",
    ...
    isValid() {
        ...
        if (!this.systemUserId) {
            console.warn("⚠️  IMPORT_SYSTEM_USER_ID not set - properties will not have a createdBy user")
        }

        if (!this.defaultAgencyId) {
            console.warn("⚠️  IMPORT_DEFAULT_AGENCY_ID not set - properties will not be assigned to an agency")
        }

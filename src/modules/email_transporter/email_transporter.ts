import { translation } from "../../utils/helpers/translations.js"
import { tryCatch } from "../../utils/helpers/try_catch.js"
import fs from "fs"
...
const templatePath = (locale: string): string =>
    path.join(process.cwd(), `/public/email_templates/email_template_default_${locale}.hbs`)
const templatePropertiesPath = (locale: string): string =>
    path.join(process.cwd(), `/public/email_templates/email_template_with_properties_${locale}.hbs`)

const getTemplate = (locale: string): string => fs.readFileSync(templatePath(locale), "utf8")
const getTemplateWithProperties = (locale: string): string => fs.readFileSync(templatePropertiesPath(locale), "utf8")

const compileTemplate = (locale: string): handlebars.TemplateDelegate => handlebars.compile(getTemplate(locale))
const compileTemplateWithProperties = (locale: string): handlebars.TemplateDelegate =>
    handlebars.compile(getTemplateWithProperties(locale))
...
export const EmailTransporter = async (locale = "mk") => {
    ...
    return {
        sendVerificationEmail: async (email: string, token: string, redirectUrl?: string) => {

...
const templatePropertiesPath = (locale) => path.join(process.cwd(), `/public/email_templates/email_template_with_properties_${locale}.hbs`);
...
export const EmailTransporter = async (locale = "mk") => {
    ...
    return {
        sendVerificationEmail: async (email, token, redirectUrl) => {
            let confirmLink = `${domain}/${locale}/auth/verifikacija-na-korisnik?token=${token}`;
            if (redirectUrl)
                confirmLink += `&redirectUrl=${encodeURIComponent(redirectUrl)}`;
